...
TEST_P(SimulatorTest, FullGameSimulation) {
    ...
    while (std::getline(inputFile, line)) {
        ...
        if (type == "id") {
            ...
            events.push_back({.eventType = EventType::Play, .data = play});
        } else if (type == "sub") {
            ...
            sub.pos =
                std::stoi(fields.at(5)); // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
            events.push_back({.eventType = EventType::Substitution, .data = sub});
        } else if (type == "com") {
            events.push_back({.eventType = EventType::Comment, .data = fields.at(1)});
        } else if (type == "radj") {
            RunnerAdjustmentInfo radj;
            radj.playerId = fields.at(1);
            radj.base = std::stoi(fields.at(2));
            events.push_back({.eventType = EventType::RunnerAdjustment, .data = radj});
        } else if (type == "badj") {
            BatterAdjustmentInfo badj;
            badj.playerId = fields.at(1);
            badj.hand = fields.at(2).at(0);
            events.push_back({.eventType = EventType::BatterAdjustment, .data = badj});
        } else if (type == "padj") {
            PitcherAdjustmentInfo padj;
            padj.playerId = fields.at(1);
            padj.hand = fields.at(2).at(0);
            events.push_back({.eventType = EventType::PitcherAdjustment, .data = padj});
        } else if (type == "data") {
            DataRecord data;
